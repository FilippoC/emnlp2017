...
int main(int argc, char **argv)
{
    ...
    desc.add_options()
        ...
        ("pos-dim", po::value<int>(&pos_dim)->default_value(50))
    ...
    NeuralHeadTaggerSettings nn_settings;
    ...
    nn_settings.predict_spine = true;
    nn_settings.n_spine = spine_settings.tpl_dict.size();
    ...
    NeuralHeadTagger<dynet::LSTMBuilder> rnn(model, nn_settings, node_rnn_settings);
    ...
    save_object(model_path + ".head_tagger.nn_settings", nn_settings);
    save_object(model_path + ".head_tagger.rnn_settings", node_rnn_settings);
    ...
    for (unsigned iteration = 0 ; iteration <= n_iteration ; ++iteration)
    {
        ...
        for (const IntSentence& sentence : train_data)
        {
            ...
            rnn.compute(
                ...
                [&] (unsigned index, dynet::expr::Expression& expr) -> void
                {
                    if (probabilistic)
                    {
                        // the root spine in the last one
                        int tpl = (sentence[index+1].head == 0 ? spine_settings.tpl_dict.size() : sentence[sentence[index+1].head].tpl);
                        errs.push_back(pickneglogsoftmax(expr, tpl));
                    ...
                    {
                        throw std::runtime_error("Use probabilistic only");
                        /*
                        ...
                        {
                            if ((int) i != sentence[index+1].tpl)
                                vec[i] += 1.0;
                        }

                        int predicted = std::distance(std::begin(vec), std::max_element(std::begin(vec), std::end(vec)));
                        if (predicted == sentence[index+1].tpl)
                        {
                            n_correct += 1;
                        }
                        else
                        {
                            errs.push_back(dynet::expr::pick(expr2, predicted) - pick(expr2, sentence[index+1].tpl));
                        }
                        */
                    }
                },
        ...
        save_object(model_path + ".head_tagger.param." + std::to_string(iteration), model);
        ...
        if (eval_on_dev)
        {
            ...
            std::cerr << " -> Precision: " << n_good_pos / (double) n_total_pos << std::endl;
    ...
    if (eval_on_dev)
    {
        ...
        boost::filesystem::copy_file(
                model_path + ".head_tagger.param." + std::to_string(max),
                model_path + ".head_tagger.param",
    ...
    else
    {
        ...
        save_object(model_path + ".head_tagger.param", model);
    }
}
