...
BOOST_CLASS_VERSION(NeuralTaggerSettings, 1)

template<class RNNBuilder>
struct NeuralTagger
{
    public:
        const NeuralTaggerSettings settings;
    ...
    explicit NeuralTagger(
        dynet::Model &model, 
        NeuralTaggerSettings t_settings,
        ...
        pos_rnn(model, pos_rnn_settings, (settings.pos_input ? settings.word_dim + settings.pos_dim : settings.word_dim))
    {
        ...
        p_hidden_layer_word = model.add_parameters({output_dim, pos_rnn.output_dim()});
        p_hidden_bias = model.add_parameters({output_dim});
